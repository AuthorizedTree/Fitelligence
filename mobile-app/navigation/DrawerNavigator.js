...
export default function Profile({ navigation }) {
  ...
  // Logout: navigate to Sign In in parent navigator
  const handleLogout = () => {
    Alert.alert('Log out?','You will need to sign in again.',[
      { text:'Cancel', style:'cancel' },
      {
        ...
        style:'destructive',
        onPress: () => {
          // clear profile optionally: AsyncStorage.removeItem(PROFILE_KEY);
          // go up to root navigator and navigate to Sign In
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.pwdRow}>
        ...
        <TouchableOpacity onPress={()=>setShowStored(v=>!v)} style={styles.showBtn}><Text style={styles.showTxt}>{showStored?'Hide':'Show'}</Text></TouchableOpacity>
      </View>

      <TouchableOpacity style={styles.resetToggle} onPress={()=>setShowReset(v=>!v)}>
...
const styles = StyleSheet.create({
  ...
  showBtn:{marginLeft:8},
  showTxt:{color:'#467fd0',fontWeight:'500'},
